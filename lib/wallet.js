...
Wallet.prototype._scanAddresses = function () {
  ...
  .then(function (unspents) {
    ...
    var diff = bitcore.Unit.fromSatoshis(newBalance - prevBalance).toBTC()
    if (diff > 0) { diff = '+' + diff.toString() }
    logger.verbose('unspents updated (total balance: %s, %s)',
                   bitcore.Unit.fromSatoshis(newBalance).toBTC(),
                   diff.toString())
