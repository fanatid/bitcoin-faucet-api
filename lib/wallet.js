...
Wallet.prototype._scanAddresses = function () {
  ...
  Promise.all(_.keys(self.addresses).map(function (address) {
    return provider.getUnspents(address)
  }))
  .then(function (unspents) {
    var unspents = _.zipObject(self.unspents.map(function (unspent) {
      return [unspent.txId + unspent.vout, true]
    }))

    return _.flatten(unspents).reduce(function (promise, unspent) {
      var key = unspent.txId + unspent.vout
      if (unspents[key] !== undefined) {
        return promise
      }
...
/**
 * @param {number} value
 * @return {Array.<Wallet~UnspentObject>}
 */
Wallet.prototype._selectUnspents = function (value) {
...
Wallet.prototype.sendTo = function (recipients) {
  var tx = bitcore.Transaction().change(this.getRandomAddress())
  recipients.forEach(function (recipient) {
    tx = tx.to(recipient.address, recipient.value)
  })

  var unspents = this._selectUnspents(tx.outputAmount)
